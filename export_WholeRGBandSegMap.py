...
def export_center_png(image_save_path, sequence_Motor) :
    bpy.ops.object.select_all(action='DESELECT')
...
def export_zivid_png(image_save_path, sequence_Motor) :
    bpy.ops.object.select_all(action='DESELECT')
        ...
        for dirs in List_motor :
           ...
           # scan(save_dir = save_dir_whole, save_numpy = True, Motor_type = Motor_type, scanner = bpy.data.objects['Camera'], sequence_Motor = k[1])
            if perspective == 'center':
                export_center_png(save_dir_whole, k[1])
            else:
                # set camera's position for zivid perspective
                ...
                with open(camera_position_path, 'w+', newline = '') as f:
                    ...
                    for s in camera_position_now:
                        p += s + ' '
                    f.writelines(p)
                export_zivid_png(save_dir_whole, k[1])
